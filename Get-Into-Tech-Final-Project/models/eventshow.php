<?php
  class Eventshow {
    ...
    //NULL is used below. If NULL is used then the ALL function below does NOT need '', '', '', at the end of the Eventshow instance. If NULL is removed then it DOES require the '', '', ''
    public function __construct($eventID,$eventName,$eventDate, $eventTime, $eventRunningTime, $eventDescription,$eventURL,$eventBookingURL,$eventPrice,$venueID_FK,$authorID_FK,$imageURL,$videoURL, $venueName=NULL,$venueStreet=NULL,$venuePostcode=NULL, $venueLocation=NULL) {
...
//    }
    ...
    //function to insert venue address to event list
    public static function find($eventID) {
      $db = Db::getInstance();
      //use intval to make sure $id is an integer
      $eventID = intval($eventID);
      $req = $db->prepare('SELECT * FROM eventshow inner join venueaddress on eventshow.venueID_FK = venueaddress.venueID where eventshow.eventID = :eventID');
      //the query was prepared, now replace :id with the actual $id value
      $req->execute(array('eventID' => $eventID));
      $eventshow = $req->fetch();
if($eventshow){
      return new Eventshow($eventshow['eventID'], $eventshow['eventName'], $eventshow['eventDate'], $eventshow['eventTime'], $eventshow['eventRunningTime'], $eventshow['eventDescription'], $eventshow['eventURL'], $eventshow['eventBookingURL'], $eventshow['eventPrice'], $eventshow['venueID_FK'], $eventshow['authorID_FK'], $eventshow['imageURL'], $eventshow['videoURL'], $eventshow['venueName'], $eventshow['street'], $eventshow['postcode'], $eventshow['location']);
    }
    else
    {
        //replace with a more meaningful exception
